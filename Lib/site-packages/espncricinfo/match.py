...
from codebase.settings import DATA_LOCATION, MATCH_JSONS, HTMLS, ANALYSIS
import os
import pickle
...
class Match(object):
    ...
    def __init__(self, match_id, try_local=True, serialize=False, save=True):
        ...
        self.json = self.get_json(try_local=try_local, save=save, serialize=serialize)
        self.html = self.get_html(try_local=try_local, save=save, serialize=serialize)
    ...
    def load_data(self, suffix, data_folder=DATA_LOCATION, subfilepath='', file_ext='json'):
        if os.path.exists(os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.p')):
                with open(os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.p'), 'rb') as jf:
                    logger.info(f"Loading data from {os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.p')}")
                    return pickle.load(jf)
        elif os.path.exists(os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.{file_ext}')):
                with open(os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.{file_ext}'), 'r', encoding='utf-8') as jf:
                    logger.info(f"Loading data from {os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.{file_ext}')}")
                    if file_ext == 'json':
                        return json.load(jf)
                    else:
                        return jf.read()
    ...
    def save_data(self, data, suffix, data_folder=DATA_LOCATION, subfilepath='', file_ext='json', serialize=True):
        if serialize:
            with open(os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.p'), 'wb') as f:
                logger.info(f"Saving data to {os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.p')}")
                pickle.dump(data, f)
        else: 
            with open(os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.{file_ext}'), 'w', encoding='utf-8') as f:
                logger.info(f"Saving data to {os.path.join(data_folder, subfilepath, f'{self.match_id}_{suffix}.{file_ext}')}")
                if file_ext == 'json':
                    f.write(json.dumps(data))
                else:
                    f.write(data)

    def get_json(self, try_local=True, save=True, serialize=True):
        if try_local:
            data = self.load_data(suffix='match_jsons', data_folder=MATCH_JSONS)
            if data:
                return data
            # if os.path.exists(os.path.join(MATCH_JSONS, f'{self.match_id}_match_json.json')):
            #     with open(os.path.join(MATCH_JSONS, f'{self.match_id}_match_json.json'), 'r') as jf:
            #         return json.load(jf)
        ...
        if r.status_code == 404:
            ...
            response = r.json()
            if save:
                self.save_data(response, suffix='match_jsons', data_folder=MATCH_JSONS, serialize=serialize)
                # with open(os.path.join(MATCH_JSONS, f'{self.match_id}_match_json.json'), 'w') as f:
                #     f.write(json.dumps(response))
            return response

    def get_html(self, try_local=True, save=True, serialize=True):
        if try_local:
            data = self.load_data(suffix='html', data_folder=HTMLS, file_ext='html')
            if data:
                return BeautifulSoup(data, 'html.parser')
            # if os.path.exists(os.path.join(HTMLS, f'{self.match_id}.html')):
            #     with open(os.path.join(HTMLS, f'{self.match_id}.html'), 'r') as jf:
            #         return BeautifulSoup(jf, 'html.parser')
        ...
        if r.status_code == 404:
            ...
            if save:
                self.save_data(r.text, suffix='html', data_folder=HTMLS, file_ext='html', serialize=serialize)
                # with open(os.path.join(HTMLS, f'{self.match_id}.html'), 'w') as f:
                #     f.write(r.text)
