...
class MatchData(Match):
    ...
    def __init__(self, match_id, try_local=True):
        super().__init__(match_id, try_local)
    ...
    def get_detailed_comms_faster(self, try_local=True, save=True):
        ...
        """
        logger.info(f'{self.match_id}: Loading match comms')
        try:
            if try_local:
            ...
            if save:
                self.save_data(data=full_comms, suffix='full_comms')
            return full_comms
        except TypeError:
            logger.warning(f'Match {self.match_id}, {self.match_title} does not have commentary data')
        except IndexError:
